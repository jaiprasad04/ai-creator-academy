...
- Every module points at an app via its entry in the track's `apps.json` (`status: "existing"` with a `repo_link`, or `status: "needed"`) — never duplicate app code inline.
