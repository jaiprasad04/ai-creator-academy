...
2. Add an entry for it in the track's `apps.json` (`{"status": "existing", "repo_link": "..."}` if a working app already demonstrates this, or `{"status": "needed", "repo_link": null}` if not).
